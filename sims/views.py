...
def index(request):
    ...
    print(sql)
    conn = MySQLdb.connect(host="localhost", user="root", passwd="1234", db="sms", charset='utf8')
    with conn.cursor(cursorclass=MySQLdb.cursors.DictCursor) as cursor:
        cursor.execute(sql)
        students = cursor.fetchall()
    return render(request, 'templates/student/index.html', {'students': students,
...
def add(request):
    if request.method == 'GET':
        return render(request, 'templates/student/add.html')
    else:
...
def edit(request):
    if request.method == 'GET':
        ...
        return render(request, 'templates/student/edit.html', {'student': student})
    else:
